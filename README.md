...
Wrangler now supports parsing byte sizes and time durations with various units. This makes it easier to work with data that contains file sizes, network transfer sizes, or time intervals.
...
A new `aggregate-stats` directive is available to aggregate byte sizes and time durations:

```sql
aggregate-stats :size_column :time_column total_size total_time [size-unit UNIT] [time-unit UNIT] [aggregation-type TYPE]
```
...
- `total_size`: Target column for total size
- `total_time`: Target column for total time
- `size-unit`: (Optional) Output unit for size (B, KB, MB, GB, TB, PB, EB)
- `time-unit`: (Optional) Output unit for time (ns, µs, ms, s, m, h, d, y)
- `aggregation-type`: (Optional) Type of aggregation (total, average)

Example:

```sql
aggregate-stats :data_size :response_time total_size_mb total_time_sec size-unit MB time-unit s
```

This will:

1. Read byte sizes from the `data_size` column
2. Read time durations from the `response_time` column
3. Calculate the total size in megabytes
4. Calculate the total time in seconds
5. Store results in `total_size_mb` and `total_time_sec` columns
